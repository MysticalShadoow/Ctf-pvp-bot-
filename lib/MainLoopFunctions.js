...
async function handleAttackFlagCarrier(bot, rgCtfUtils, opponents, teamMates) {
    ...
    if (!flagLocation) {
        ...
        if (opponentWithFlag) {
            ...
            bot._client.write("position", { ...this.bot.entity.position.offset(0, 0.1625, 0), onGround: false });
            bot._client.write("position", { ...this.bot.entity.position.offset(0, 4.0e-6, 0), onGround: false });
            bot._client.write("position", { ...this.bot.entity.position.offset(0, 1.1e-6, 0), onGround: false });
            bot._client.write("position", { ...this.bot.entity.position, onGround: false });
